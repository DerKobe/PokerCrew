...
const DICT = {
  de: {
    ...
    lobby: {
      ...
      sum0: (p) => `<b>${p.title}</b> · Filz ${p.felt} · Rand ${p.rim} · Karten ${p.back}`,
      stack: 'Startchips',
      ...
      bots: 'Freie Plätze mit Bots auffüllen',
      botsHint: '– beim Start setzen sich Bots auf alle freien Plätze',
      botSeat: 'Bot (oder du)',
      plusBots: (p) => `+ ${p.n} ${p.n === 1 ? 'Bot' : 'Bots'}`,
    ...
    profile: {
      rock: { name: 'Fels', desc: 'Defensiv: spielt kaum Hände und blufft fast nie.' },
      station: { name: 'Calling Station', desc: 'Geht fast immer mit, erhöht selten.' },
      tag: { name: 'Hai', desc: 'Schlau: tight-aggressiv und kennt die Pot Odds.' },
      lag: { name: 'Aggressiv', desc: 'Spielt viele Hände und macht ständig Druck.' },
      maniac: { name: 'Maniac', desc: 'Erhöht alles und blufft ohne Ende.' },
    ...
    settings: {
      title: 'Einstellungen',
      close: 'Schließen',
      gadgets: 'Gadgets anzeigen',
      gadgetsHint: 'Zigarre, Vape & Co. am Tisch – mit Animationen und Geräuschen',
      trophies: 'Trophäen anzeigen',
      trophiesHint: 'Die kleinen Auszeichnungen neben den Karten',
      noTopple: 'Meinen Stapel schützen',
      noToppleHint: 'Niemand kann deine Chips umwerfen',
      note: 'Diese Einstellungen gelten nur für dich.',
    },
    ...
    log: {
      ...
      botsJoin: (p) => `🤖 Bots am Tisch: ${p.names.join(', ')}`,
  ...
  en: {
    ...
    lobby: {
      ...
      game: 'Game',
      cardBack: 'Card back',
      sum0: (p) => `<b>${p.title}</b> · ${p.felt} felt · ${p.rim} rim · ${p.back} cards`,
      ...
      bots: 'Fill empty seats with bots',
      botsHint: '– bots take every free seat when the tournament starts',
      botSeat: 'Bot (or you)',
      plusBots: (p) => `+ ${p.n} ${p.n === 1 ? 'bot' : 'bots'}`,
      ready: (p) => `${p.n} ${p.n === 1 ? 'player' : 'players'} ready${p.spec ? ` · ${p.spec} spectator${p.spec > 1 ? 's' : ''}` : ''}`,
    ...
    profile: {
      rock: { name: 'Rock', desc: 'Defensive: plays very few hands and hardly ever bluffs.' },
      station: { name: 'Calling Station', desc: 'Calls almost everything, rarely raises.' },
      tag: { name: 'Shark', desc: 'Smart: tight-aggressive and knows the pot odds.' },
      lag: { name: 'Aggressive', desc: 'Plays lots of hands and keeps the pressure on.' },
      maniac: { name: 'Maniac', desc: 'Raises everything and bluffs non-stop.' },
    ...
    settings: {
      ...
      gadgetsHint: 'Cigar, vape & co. at the table – with animations and sounds',
      trophies: 'Show trophies',
      trophiesHint: 'The little awards next to the cards',
      noTopple: 'Protect my stack',
      noToppleHint: 'Nobody can knock your chips over',
      note: 'These settings only apply to you.',
    },
    odds: { outs: (p) => `${p.n} ${p.n === 1 ? 'out' : 'outs'}`, split: (p) => `Split ${p.p}%` },
    banner: {
      wins: 'wins',
    ...
    log: {
      ...
      botsJoin: (p) => `🤖 Bots at the table: ${p.names.join(', ')}`,
